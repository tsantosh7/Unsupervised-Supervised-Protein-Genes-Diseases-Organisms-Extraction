...
dataset = pd.read_csv(file_path+'train.csv', sep = '\t', names = ['pmcid', 'text', 'ner'])
...
for iter_, row in tqdm(dataset.iterrows(), total = len(dataset)):
    ...
    sector_CSV.to_csv(file_path+'CD_GP_DS_OG_train.csv',encoding='utf-8', index=False,  mode='a', header=False)
